...
$scope.save_site = function() 
	...
	$http.post('get_all_officer.php')
	.success(function(data, status, headers, config) 
	{
			$scope.all_staff_details = data.details;
    });
	/****************************************************************************/
/************************** Delete User  *********************************/
/****************************************************************************/
	...
	$scope.delete_officer = function(user_id) 
	{		
        $http.post('delete_officer.php', 
		{
		'id': user_id
		})
		.success(function(data, status, headers, config) 
		{
			...
			{
				alert("Deleted Successful");
				window.location = "admin_view_officer.html";
				return;
			}
			else if(data.success == 0)
			{
	...
	$http.post('status_all_count.php')
	.success(function(data, status, headers, config) 
	{
			$scope.all_status_details = data.details;
    });
	
	$http.post('complaint_all.php')
	.success(function(data, status, headers, config) 
	{
...
$scope.update_officer = function(user_id,name,email,password,mobile,place,dept) 
	{
		window.location = "update_officer.html";
		...
		$cookieStore.put("cook_place",place);
		$cookieStore.put("cook_dept",dept);
		return;
	}	
	...
	$scope.cook_place = $cookieStore.get("cook_place");
	$scope.cook_dept = $cookieStore.get("cook_dept");


	$scope.save_officer = function() 
	{		
		$http.post('save_officer.php',{
		 'name':$scope.cook_name,'email':$scope.cook_email, 'password':$scope.cook_password,
		 'mobile': $scope.cook_mobile, 'place': $scope.cook_place,'dept': $scope.cook_dept, 
		 'user_id': $scope.cook_user_id})
		.success(function(data, status, headers, config) 
		{
			if(data.success == 1)
			{
