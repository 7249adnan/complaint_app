...
app.controller("myappCtrl", function ($scope, $cookieStore, $cookies, $http) {
  ...
  $http.post("get_site.php").success(function (data, status, headers, config) {
    ...
    $http
      ...
      .success(function (data, status, headers, config) {
        if (data.success == 1) {
          ...
          alert("OTP Sent successfully to Admin");
  ...
  /****************************************************************************/
  // sign in button
  //, 'place':$scope.place,'dept':$scope.dept
